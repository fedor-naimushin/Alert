...
{
    Task<TAggregateRoot> GetById(long id, CancellationToken cancellationToken);
    Task<TAggregateRoot> GetById(object[] keyValues, CancellationToken cancellationToken);
    ...
    Task<IReadOnlyList<TResult>> QueryAsync<TResult>(Expression<Func<IQueryable<TAggregateRoot>, IQueryable<TResult>>> predicate, CancellationToken cancellationToken);
}